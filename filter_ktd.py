...
# 1. Update Controller
controller_path = 'app/Http/Controllers/KelompokTaniController.php'
...
old_index = """    public function index()
    {
        ...
        if ($user->role === 'master_wilayah') {
            ...
            $ktds = KelompokTani::with('masterAdmin')->where('wilayah_id', $user->wilayah_id)->latest()->get();
        } else {
            // super_admin bisa melihat semua KTD dan perlu list wilayah untuk tambah KTD
            $ktds = KelompokTani::with(['wilayah', 'masterAdmin'])->latest()->get();
...
new_index = """    public function index(Request $request)
    {
        ...
        if ($user->role === 'master_wilayah') {
            ...
            $query->with('masterAdmin')->where('wilayah_id', $user->wilayah_id);
        } else {
            // super_admin bisa melihat semua KTD dan perlu list wilayah untuk tambah KTD
...
# 2. Update View
view_path = 'resources/views/admin/ktd/index.blade.php'
...
"""
view_content = view_content.replace('<!-- Table Data KTD -->', filter_ui + '<!-- Table Data KTD -->')
...
old_inner_tambah = """<div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative z-10">"""
...
view_content = view_content.replace('<h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Tambah Kelompok Tani (Akun KTD)</h3>', '<div class="border-b border-gray-100 pb-4 mb-4"><h3 class="text-lg leading-6 font-bold text-gray-900" id="modal-title">Tambah Kelompok Tani (Akun KTD)</h3></div>')
...
old_inner_edit = """<div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative z-10">"""
...
view_content = view_content.replace('<h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Edit Kelompok Tani Dewasa</h3>', '<div class="border-b border-gray-100 pb-4 mb-4"><h3 class="text-lg leading-6 font-bold text-gray-900" id="modal-title">Edit Kelompok Tani Dewasa</h3></div>')

# 3. Add Wilayah TomSelect classes for Super Admin (since only super admin has wilayah_id dropdown in modals)
old_tambah_select = '<select name="wilayah_id" id="wilayah_id" required class="mt-1 focus:ring-[#5C52E7] focus:border-[#5C52E7] block w-full shadow-sm sm:text-sm border-gray-300 rounded-xl px-4 py-2 border outline-none transition-colors">'
new_tambah_select = '<select name="wilayah_id" id="add_wilayah_id" required class="mt-1 block w-full">'
view_content = view_content.replace(old_tambah_select, new_tambah_select)

old_edit_select = '<select name="wilayah_id" id="edit_wilayah_id" required class="mt-1 focus:ring-[#5C52E7] focus:border-[#5C52E7] block w-full shadow-sm sm:text-sm border-gray-300 rounded-xl px-4 py-2 border outline-none transition-colors">'
new_edit_select = '<select name="wilayah_id" id="edit_wilayah_ts" required class="mt-1 block w-full">'
view_content = view_content.replace(old_edit_select, new_edit_select)

# 4. Fix bottom script
# We will just append the TomSelect init code to the bottom before @endsection
